    ...
    pub fn debug_print_spans(&self, iv: Interval) {
        for (id, layer) in &self.layers {
            ...
            if spans.iter().next().is_some() {
                println!("scopes for layer {:?}:", id);
                for (iv, val) in spans.iter() {
                    println!("{}: {:?}", iv, layer.stack_lookup[*val as usize]);
                }
                println!("styles:");
        ...
        for stack in scopes {
            let scopes = stack
                ...
                .filter(|result| match *result {
                    Err(ref err) => {
                        println!("failed to resolve scope {}\nErr: {:?}", &stack.join(" "), err);
                        false
                    }
                    _ => true,
                })

...
use rope::{Cursor, DeltaBuilder, Interval, LinesMetric, Rope, RopeDelta};

use crate::backspace::offset_for_delete_backwards;
use crate::line_offset::{LineOffset, LogicalLines};
use crate::linewrap::Lines;
use crate::movement::region_movement;
use super::{Motion, Quantity};
...
pub fn duplicate_line(base: &Rope, regions: &[SelRegion]) -> RopeDelta {
    let mut builder = DeltaBuilder::new(base.len());
    ...
    for (start, end) in to_duplicate {
        ...
        if end == base.len() {
            builder.replace(iv, Rope::from(LINE_ENDING))
        }
    }
...
pub fn delete_backward(base: &Rope, regions: &[SelRegion]) -> RopeDelta {
    ...
    let mut builder = DeltaBuilder::new(base.len());
    for region in regions {
        let start = offset_for_delete_backwards(&region, base);
...
pub(crate) fn delete_by_movement(
    ...
    for &r in regions {
        if r.is_caret() {
            let new_region = region_movement(motion, quantity, r, lines, height, base, true);
...
pub fn insert_newline(base: &Rope, regions: &[SelRegion]) -> RopeDelta {
    insert(base, regions, LINE_ENDING)
}

pub fn insert_tab(base: &Rope, regions: &[SelRegion]) -> RopeDelta {
    let mut builder = DeltaBuilder::new(base.len());
    let const_tab_text = get_tab_text(None);
    ...
    for region in regions {
        ...
        if line_range.len() > 1 {
            ...
            let mut tab_size = TAB_SIZE; 
            tab_size = tab_size - (col % tab_size);
            let tab_text = get_tab_text(Some(tab_size));
...
pub fn modify_indent(
    ...
    let tab_text = get_tab_text(None);
    for region in regions {
        let line_range = LogicalLines.get_line_range(base, region);
        for line in line_range {
            lines.insert(line);
...
fn get_tab_text(tab_size: Option<usize>) -> &'static str {
    let tab_size = tab_size.unwrap_or(TAB_SIZE);
    let tab_text = n_spaces(tab_size);
